...
public class SpreadSheet extends JFrame {
    ...
    private class Cell {
        ...
        String formulaText; // the formula to show
	...
	public void setCell(int row, int col, String field) {
		...
		cells[row][col].formulaText = formatNumber(field);
	}
    ...
    public void evaluate(int r, int c, int depth) {
        ...
        if (formula.length() > 0 && formula.charAt(0) == '=') {
            ...
            cellsTF[r][c].setText(cells[r][c].formulaText);
    ...
    public void cellChanged(JTextField cell) {
        ...
        String ft = cells[r][c].formulaText;
        // update the formula if it is just a value (non-'=' prefix)
        if (f.length() <= 0 || f.charAt(0) != '=') {
        	String text = cell.getText();
        	text = (!f.equals(text) ? (!ft.equals(text)? text:f): text);
            cells[r][c].formula = text;
            cells[r][c].formulaText = formatNumber(text);
    ...
    //GASE - Method Added
  	private boolean isFloatingNumber(String str) {
  		if (str == null) {
  			return false;
  		}
  ...
  //GASE - Method Added
  	private boolean isIntNumber(String str) {
  		return !this.isFloatingNumber(str);
  	}
    ...
    private String formatNumber(String text) {
    	...
    	if(isIntNumber(text)) {
        	newText = String.format("%.4g", doubleValue);
    	}
    	if(isFloatingNumber(text)) {
    		newText = String.format("%.6f", doubleValue);
    	}    		    	
		return newText;
	}
